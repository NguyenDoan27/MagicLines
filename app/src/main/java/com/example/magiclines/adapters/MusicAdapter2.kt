...
import com.example.magiclines.callbacks.AudioCallback
...
class MusicAdapter2(private val context: Context,var currentPosition: Int, val onItemClick: (Int) -> Unit):
    ListAdapter<Audio, MusicAdapter2.ViewHolder>(AsyncDifferConfig.Builder<Audio>(AudioCallback())
        .setBackgroundThreadExecutor(Executors.newSingleThreadExecutor()).build()){
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): MusicAdapter2.ViewHolder {
        return ViewHolder(MusicItemBinding.inflate(LayoutInflater.from(context), parent, false))
    }

    override fun onBindViewHolder(holder: MusicAdapter2.ViewHolder, position: Int) {
